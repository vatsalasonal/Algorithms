...
public class Maximal_Square_Part2_221 {
	...
	public static void main(String[] args) {
		...
		int squareArea = findSquareArea(matrix, max);
		System.out.println("The square area is : "+squareArea);

	}

	public static int findSquareArea(char[][] matrix, int max) {
		if (matrix.length == 0) {
			return 0;
		}
	...
	public static int[][] findArea(int[][] newArray, int[][] arr, int row, int col, int max) {
		...
		if (newArray[row][col] == 0) {
			arr[row][col] = 0;
			arr[row][col] = findMaximalSquare(newArray, arr, row, col - 1);
			arr[row][col] = findMaximalSquare(newArray, arr, row - 1, col);
		}
